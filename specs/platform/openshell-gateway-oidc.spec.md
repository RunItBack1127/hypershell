...
This specification defines per-gateway OIDC authentication for OpenShell gateways. OIDC enables CLI users and external clients to authenticate via Bearer tokens from an identity provider (e.g., Keycloak). OIDC is the primary authentication mechanism for HyperShell gateways in production, while sandbox supervisors authenticate via mTLS client certificates.
...
- The Keycloak realm SHALL include an `openshell-cli` client
